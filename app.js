...
let currentMatches = [];
let adminLoggedIn = sessionStorage.getItem("vblAdmin") === "true";
...
function matchEditorHtml(list){
  const rows=list.map((m,i)=>`<div class="match-editor-row" data-index="${i}">
...
function readMatchEditor(){
  return [...matchesEditorWrap.querySelectorAll(".match-editor-row")].map(row=>{
    ...
    m.id = row.dataset.matchId || Date.now()+Math.random();
    return m;
  });
}
function showAdmin(){
  adminLoggedIn=true; sessionStorage.setItem("vblAdmin","true");
  loginView.classList.add("hidden"); editorView.classList.remove("hidden");
  editorHtml(currentTeams); matchEditorHtml(currentMatches);
}
function hideAdmin(){
  adminLoggedIn=false; sessionStorage.removeItem("vblAdmin"); loginView.classList.remove("hidden"); editorView.classList.add("hidden");
}
...
$("saveTable").onclick=async()=>{try{const list=readEditor();await set(teamsRef,Object.fromEntries(list.map(t=>[String(t.id),t])));alert("Tabelle gespeichert. Alle sehen die Änderung.");}catch(e){alert("Speichern fehlgeschlagen: "+e.message);}};
$("addMatch").onclick=()=>{const list=readMatchEditor();list.push({id:Date.now(),status:"next",home:"",away:"",kickoff:"",homeScore:0,awayScore:0,minute:"",date:""});matchEditorHtml(list);};
...
$("saveMatches").onclick=async()=>{try{const list=readMatchEditor().map(m=>{const id=String(m.id);delete m.id;return {id,data:m};});const payload=Object.fromEntries(list.map(x=>[x.id,x.data]));await set(matchesRef,payload);alert("Spiele gespeichert. Alle sehen die Änderung.");}catch(e){alert("Speichern fehlgeschlagen: "+e.message);}};
