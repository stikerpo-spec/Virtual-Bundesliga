...
function updateStandTime(){
  const el = $("updatedAt");
  if (!el) return;
  el.textContent = `Stand: ${new Date().toLocaleTimeString("de-DE", {hour:"2-digit", minute:"2-digit", second:"2-digit"})}`;
}
function updateStatus(online,text){
  $("liveDot").classList.toggle("live",online);
  $("liveText").textContent=text;
  if (online) updateStandTime(); else $("updatedAt").textContent="–";
}
...
// Die angezeigte Uhrzeit oben rechts bleibt auch ohne Datenänderung aktuell.
