...
$("loginBtn").onclick=async()=>{
  ...
  if(code !== "Stikeli"){$("loginError").textContent="Falscher Admin-Code.";return;}
  try{
    await signInWithEmailAndPassword(auth, ADMIN_EMAIL, "Stikeli");
    $("adminCode").value="";
  }catch(e){
    if(e?.code === "auth/user-not-found") {
      try{
        await createUserWithEmailAndPassword(auth, ADMIN_EMAIL, "Stikeli");
        $("adminCode").value="";
        return;
      }catch(createErr){
    ...
    $("loginError").textContent = "Firebase-Login konnte nicht gestartet werden.";
  }
};
