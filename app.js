...
<input data-k="logo" value="${esc(t.logo||"")}" placeholder="Logo-URL">
<button class="delete" data-action="delete" type="button">×</button>
  </div>`).join("");
  editorWrap.innerHTML = `<div class="editor-grid"><div class="editor-head"><div>ID</div><div>TEAM</div><div>TRAINER</div><div>SP</div><div>S</div><div>U</div><div>N</div><div>TORE</div><div>DIFF</div><div>PUNKTE</div><div>LOGO-URL</div><div></div></div>${rows}</div>`;
}
function readEditor(){
