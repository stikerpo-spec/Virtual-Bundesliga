...
function createExtraPublicSections() {
  ...
  if (!$("playerStatsSection")) {
    ...
    playerSection.id = "playerStatsSection";
    playerSection.className = "player-stats-section";
    const liveSection = page.querySelector(".live-games-section");
    if (liveSection) page.insertBefore(playerSection, liveSection);
    else page.appendChild(playerSection);
  }

  if ($("playerStatsSection") && !$("playerStatsView")) {
    $("playerStatsSection").innerHTML = `
      <div class="section-title-row">
        <div>
          <span class="section-kicker">SPIELER</span>
          <h2>Spielerstatistiken</h2>
          <p class="section-sub">Torschützen, Vorlagen, Einsätze und Karten der Liga</p>
        </div>
      </div>
      <div class="player-stats-controls">
        <div class="player-stat-tabs" role="tablist" aria-label="Statistik auswählen">
          <button class="stat-tab active" data-player-stat="goals" type="button">⚽ Tore</button>
          <button class="stat-tab" data-player-stat="assists" type="button">🎯 Vorlagen</button>
          <button class="stat-tab" data-player-stat="appearances" type="button">👕 Einsätze</button>
          <button class="stat-tab" data-player-stat="yellowCards" type="button">🟨 Gelb</button>
          <button class="stat-tab" data-player-stat="redCards" type="button">🟥 Rot</button>
        </div>
        <label class="player-team-filter">Team
          <select id="playerTeamFilter"><option value="">Alle Teams</option></select>
        </label>
      </div>
      <div id="playerStatsView"></div>
    `;
  }
...
const PLAYER_STAT_DEFS = {
  goals: { label: "Tore", icon: "⚽" },
  assists: { label: "Vorlagen", icon: "🎯" },
  appearances: { label: "Einsätze", icon: "👕" },
  yellowCards: { label: "Gelbe Karten", icon: "🟨" },
  redCards: { label: "Rote Karten", icon: "🟥" }
};

let selectedPlayerStat = "goals";

function playerNumber(value) {
  return Math.max(0, Number(value) || 0);
}

function playerTeamObject(player) {
  return teamByName(player.team) || { team: player.team, logo: player.logo || "" };
}
...
function populatePlayerTeamFilter() {
  ...
  select.innerHTML = `<option value="">Alle Teams</option>` +
    currentTeams.map(t => `<option value="${esc(t.team)}">${esc(t.team)}</option>`).join("");
  select.value = currentTeams.some(t => t.team === previous) ? previous : "";
}
...
function renderPlayerStats() {
  ...
  const teamFilter = $("playerTeamFilter")?.value || "";
  const stat = PLAYER_STAT_DEFS[selectedPlayerStat] || PLAYER_STAT_DEFS.goals;

  const list = currentPlayers
    .filter(p => !teamFilter || p.team === teamFilter)
    .sort((a,b) =>
      playerNumber(b[selectedPlayerStat]) - playerNumber(a[selectedPlayerStat]) ||
      playerNumber(b.goals) - playerNumber(a.goals) ||
      String(a.name || "").localeCompare(String(b.name || ""), "de")
    );

  const nonZero = list.filter(p => playerNumber(p[selectedPlayerStat]) > 0);
  const podium = nonZero.slice(0,3);

  view.innerHTML = `
    ${podium.length ? `
      <div class="player-podium">
        ${podium.map((p,i) => `
          <div class="player-podium-card player-place-${i+1}">
            <span class="player-rank-medal">${["🥇","🥈","🥉"][i]}</span>
            <div class="player-podium-main">
              <strong>${esc(p.name || "Unbekannt")}</strong>
              <span>${esc(p.team || "–")}</span>
            </div>
            <div class="player-podium-value"><strong>${playerNumber(p[selectedPlayerStat])}</strong><span>${stat.label}</span></div>
          </div>
        `).join("")}
      </div>
    ` : `<div class="player-empty">Noch keine Werte für <b>${esc(stat.label.toLowerCase())}</b> eingetragen.</div>`}

    <div class="player-table-card">
      <div class="player-table-head">
        <span>RANG</span><span>SPIELER</span><span>TEAM</span><span>TORE</span><span>VORL.</span><span>SP.</span><span>FORM</span><span>${esc(stat.label.toUpperCase())}</span>
      </div>
      ${list.length ? list.map((p,i) => `
        <div class="player-table-row">
          <span class="player-rank">${i+1}</span>
          <span class="player-name-cell"><strong>${esc(p.name || "Unbekannt")}</strong></span>
          <span class="player-team-cell">${teamLogo(playerTeamObject(p))}<span>${esc(p.team || "–")}</span></span>
          <span>${playerNumber(p.goals)}</span>
          <span>${playerNumber(p.assists)}</span>
          <span>${playerNumber(p.appearances)}</span>
          <span class="player-extra-form">${playerNumber(p.yellowCards)}🟨 ${playerNumber(p.redCards)}🟥</span>
          <strong class="player-highlight-value">${playerNumber(p[selectedPlayerStat])}</strong>
        </div>
      `).join("") : `<div class="player-empty">Keine Spieler vorhanden.</div>`}
    </div>
  `;
}

function playerEditorHtml(list) {
  const wrap = $("playersEditorWrap");
  if (!wrap) return;
  wrap.innerHTML = `
    <div class="admin-player-editor">
      ${list.map((p,i) => `
        <div class="admin-player-card" data-player-id="${esc(p.id || `player-${i}`)}">
          <div class="admin-player-card-head">
            <div>
              <strong>${esc(p.name || "Neuer Spieler")}</strong>
              <small>${esc(p.team || "Kein Team")}</small>
            </div>
            <button class="delete" data-action="delete-player" type="button">×</button>
          </div>
          <div class="admin-player-fields">
            <label>Spieler<input data-pk="name" value="${esc(p.name || "")}" placeholder="Spielername"></label>
            <label>Team<select data-pk="team">${currentTeams.map(t => `<option value="${esc(t.team)}" ${t.team === p.team ? "selected" : ""}>${esc(t.team)}</option>`).join("")}</select></label>
            <label>Tore<input data-pk="goals" type="number" min="0" value="${playerNumber(p.goals)}"></label>
            <label>Vorlagen<input data-pk="assists" type="number" min="0" value="${playerNumber(p.assists)}"></label>
            <label>Einsätze<input data-pk="appearances" type="number" min="0" value="${playerNumber(p.appearances)}"></label>
            <label>Gelbe Karten<input data-pk="yellowCards" type="number" min="0" value="${playerNumber(p.yellowCards)}"></label>
            <label>Rote Karten<input data-pk="redCards" type="number" min="0" value="${playerNumber(p.redCards)}"></label>
            <label class="wide">Notiz<input data-pk="note" value="${esc(p.note || "")}" placeholder="z. B. Kapitän, bester Spieler ..."></label>
          </div>
        </div>
      `).join("")}
    </div>
...
function readPlayerEditor() {
  const wrap = $("playersEditorWrap");
  if (!wrap) return [];
  return [...wrap.querySelectorAll(".admin-player-card")].map((card) => {
    const p = { id: card.dataset.playerId || makePlayerId() };
    card.querySelectorAll("[data-pk]").forEach(input => {
      const key = input.dataset.pk;
      p[key] = ["goals","assists","appearances","yellowCards","redCards"].includes(key)
        ? playerNumber(input.value) : input.value.trim();
    });
    return p;
  });
}
...
function makePlayerId() {
  return `player-${Date.now()}-${Math.random().toString(36).slice(2,8)}`.replace(/[.#$\/[\]]/g,"_");
}
...
function renderPodium(sorted) {
  const podium = $("podiumStrip");
  if (!podium) return;

  const top = sorted.slice(0, 3);
  const medals = ["🥇", "🥈", "🥉"];
  const labels = ["CHAMPION", "RUNNER-UP", "TOP 3"];

  podium.innerHTML = top.map((team, index) => `
    <button
      type="button"
      class="podium-card podium-${index + 1}"
      data-team-link="${esc(team.team)}"
    >
      <span class="podium-medal">${medals[index] || ""}</span>
      <div class="podium-logo">${teamLogo(team, true)}</div>
      <div class="podium-copy">
        <span class="podium-label">${labels[index] || ""}</span>
        <strong>${esc(team.team)}</strong>
        <small>${esc(team.trainer || "")}</small>
      </div>
      <div class="podium-points">
        <strong>${Number(team.points) || 0}</strong>
        <span>P</span>
      </div>
    </button>
  `).join("");

  const teamCount = $("heroTeamCount");
  if (teamCount) teamCount.textContent = currentTeams.length;

  const matchCount = $("heroMatchCount");
  if (matchCount) matchCount.textContent = currentMatches.length;
}
...
$("addPlayer")?.addEventListener("click", () => {
  ...
  const list = readPlayerEditor();
  list.push({
    id: makePlayerId(),
    name: "NEUER SPIELER",
    team: currentTeams[0]?.team || "",
    goals: 0,
    assists: 0,
    appearances: 0,
    yellowCards: 0,
  ...
  playerEditorHtml(list);
});

$("playersEditorWrap")?.addEventListener("click", (e) => {
  if (e.target.dataset.action === "delete-player") {
    e.target.closest(".admin-player-card")?.remove();
  }
});

$("savePlayers")?.addEventListener("click", async () => {
  if (!adminLoggedIn) return alert("Bitte zuerst Admin-Code eingeben.");
  if (!playersRef) return alert("Firebase ist nicht verbunden.");
  try {
    const list = readPlayerEditor().map((p) => {
      p.id = cleanKey(p.id || makePlayerId());
      return p;
    });
    await set(playersRef, Object.fromEntries(list.map(p => [p.id, p])));
    alert("Spielerstatistiken gespeichert. Alle Besucher sehen die Änderung.");
  } catch (e) {
    console.error(e);
...
$("playerTeamFilter")?.addEventListener("change", renderPlayerStats);
document.addEventListener("click", (e) => {
  const tab = e.target.closest("[data-player-stat]");
  if (!tab) return;
  selectedPlayerStat = tab.dataset.playerStat || "goals";
  document.querySelectorAll("[data-player-stat]").forEach(btn => btn.classList.toggle("active", btn === tab));
  renderPlayerStats();
});
...
try {
  ...
  onValue(
    playersRef,
    (snap) => {
      currentPlayers = snap.exists()
        ? Object.entries(snap.val()).map(([id, data]) => ({ id, ...data }))
        : [];
      renderPlayerStats();
      if (adminLoggedIn) playerEditorHtml(currentPlayers);
    },
    (err) => {
      console.error(err);
      currentPlayers = [];
...
/* SCROLL POSITION BEI AUTO-AKTUALISIERUNG BEIBEHALTEN */
...
function saveScrollPosition() {
  try {
    ...
    console.warn("Scrollposition konnte nicht gespeichert werden:", err);
  }
}

function restoreScrollPosition() {
  try {
    const saved = sessionStorage.getItem("vbl-scroll-y");
    ...
    // Nach dem Reload warten, bis das Layout wieder aufgebaut wurde.
    ...
    console.warn("Scrollposition konnte nicht wiederhergestellt werden:", err);
  }
}

// Browser soll beim Reload nicht selbst zusätzlich springen.
