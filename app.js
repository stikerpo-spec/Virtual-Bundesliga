...
function getLiveMinute(match) {
  const status = match.status || "next";
  if (status === "paused") return "PAUSE";
  if (status === "halftime") return "HALBZEIT";
  if (status !== "live") return status === "postponed" ? "VERSCHOBEN" : "";
  if (match.autoClock === false) return match.minute ? `${match.minute}′` : "LIVE";
  if (match.kickoffAt) {
    const start = Date.parse(match.kickoffAt);
    if (!Number.isNaN(start)) {
      const mins = Math.max(0, Math.floor((Date.now() - start) / 60000));
      const cap = Number(match.matchLength) || 90;
      return `${Math.min(mins + 1, cap)}′`;
    }
  }
  return match.minute ? `${String(match.minute).replace(/′$/, "")}′` : "LIVE";
}

function derivedStandings(list) {
  const finished = currentMatches.filter(m => m.status === "finished");
  if (!finished.length) return [...list];
  const byName = new Map(list.map(t => [t.team, { ...t, sp: 0, s: 0, u: 0, n: 0, goals: "0:0", diff: 0, points: 0 }]));
  finished.forEach(m => {
    if (!byName.has(m.home) || !byName.has(m.away)) return;
    const hs = Number(m.homeScore) || 0;
    const as = Number(m.awayScore) || 0;
    const h = byName.get(m.home);
    const a = byName.get(m.away);
    h.sp++; a.sp++;
    const hg = parseGoals(h.goals), ag = parseGoals(a.goals);
    hg.for += hs; hg.against += as; ag.for += as; ag.against += hs;
    h.goals = `${hg.for}:${hg.against}`; a.goals = `${ag.for}:${ag.against}`;
    h.diff = hg.for - hg.against; a.diff = ag.for - ag.against;
    if (hs > as) { h.s++; h.points += 3; a.n++; }
    else if (hs < as) { a.s++; a.points += 3; h.n++; }
    else { h.u++; a.u++; h.points++; a.points++; }
  });
    ...
    <div class="team-profile-header">
      ...
      <div class="team-profile-main">
        ...
        <div class="team-profile-title">
          ...
          <button class="ghost team-share" type="button" data-share-team="${esc(team.team)}">Team teilen</button>
...
function renderTable(
  ...
  const sorted =
    sortTeams(derivedStandings(list));
...
function matchCard(
  ...
  const extra =
    status === "live"
      ? `<span data-live-minute="${esc(m.id || `${m.home}-${m.away}`)}">${esc(getLiveMinute(m))}</span>`
      : status === "paused" || status === "halftime"
      ? esc(status === "paused" ? "PAUSE" : "HALBZEIT")
      : status === "next"
      ? esc(m.kickoff || "")
      : status === "postponed"
      ? "Verschoben"
...
// Die Firebase-Listener aktualisieren die Seite bereits live; kein erzwungener Reload mehr.
/* =========================================================
   ENHANCED PORTAL BRIDGE
   ========================================================= */
window.VBL_APP = {
  getTeams: () => currentTeams,
  getMatches: () => currentMatches,
  getDb: () => db,
  getTeamsRef: () => teamsRef,
  getMatchesRef: () => matchesRef,
  refresh: () => {
