...
function renderTable(list){
  ...
  body.innerHTML = sorted.map((t,i)=>`<tr class="${i===0?"leader-row":""}">
    <td class="place">${i+1}.</td><td class="left team-name"><div class="team-cell">${teamLogo(t.team)}<span>${esc(t.team)}</span></div></td><td class="left">${esc(t.trainer)}</td>
...
function renderLeader(leader){
  ...
  $("leaderCard").innerHTML=`<div class="leader-label">TABELLENFÜHRER</div><div class="leader-main">${teamLogo(leader.team)}<div><div class="leader-team">${esc(leader.team)}</div><div class="leader-trainer">${esc(leader.trainer||"")}</div></div><div class="leader-points"><strong>${Number(leader.points)||0}</strong><span>PUNKTE</span></div></div>`;
}
