...
(() => {
  const state = {
    ...
    selectedSeason: "current", tableMode: "overall",
  ...
  function matchdayNumber(m){return Number(m.matchday)||1;}
  function isReturnLeg(m){const nums=matches().filter(x=>x.status!=="postponed").map(matchdayNumber);const max=Math.max(2,...nums);return matchdayNumber(m)>Math.ceil(max/2);}
  function calcTable(mode="overall"){
    const rows=new Map(teams().map(t=>[t.team,{team:t.team,trainer:t.trainer||"",sp:0,s:0,u:0,n:0,gf:0,ga:0,points:0,form:[]}]));
    let pool=matches().filter(m=>m.status==="finished");
    if(mode==="home") pool=pool.filter(m=>rows.has(m.home));
    if(mode==="away") pool=pool.filter(m=>rows.has(m.away));
    if(mode==="return") pool=pool.filter(isReturnLeg);
    pool.forEach(m=>{const s=parseScore(m),h=rows.get(m.home),a=rows.get(m.away);if(!h||!a)return;h.sp++;a.sp++;h.gf+=s.h;h.ga+=s.a;a.gf+=s.a;a.ga+=s.h;if(s.h>s.a){h.s++;a.n++;h.points+=3;h.form.push("W");a.form.push("L")}else if(s.h<s.a){a.s++;h.n++;a.points+=3;a.form.push("W");h.form.push("L")}else{h.u++;a.u++;h.points++;a.points++;h.form.push("D");a.form.push("D")}});
    return [...rows.values()].map(r=>({...r,diff:r.gf-r.ga,goals:`${r.gf}:${r.ga}`,form:r.form.slice(-5)})).sort((a,b)=>b.points-a.points||b.diff-a.diff||b.gf-a.gf||a.team.localeCompare(b.team));
  }
  function favorites(){try{return JSON.parse(localStorage.getItem("vblFavorites")||"[]")}catch(_){return[]}}
  function isFavorite(name){return favorites().includes(String(name))}
  function toggleFavorite(name){const f=new Set(favorites());f.has(name)?f.delete(name):f.add(name);localStorage.setItem("vblFavorites",JSON.stringify([...f]));toast((f.has(name)?"⭐ Favorit hinzugefügt: ":"☆ Favorit entfernt: ")+name);renderDashboard();renderTeams();renderTableMode();}
  function renderTableMode(mode=state.tableMode){
    state.tableMode=mode;const rows=calcTable(mode),bodyEl=$("standingsBody");if(!bodyEl)return;
    bodyEl.innerHTML=rows.map((t,i)=>`<tr><td class="place">${i+1}</td><td class="left"><div class="table-team-cell"><button class="star-btn ${isFavorite(t.team)?"on":""}" data-fav-team="${esc(t.team)}">★</button>${teamLogo(t)}<strong>${esc(t.team)}</strong></div></td><td class="left">${esc(t.trainer||"–")}</td><td>${t.form?.length?t.form.map(x=>`<span class="form-dot form-${x.toLowerCase()}">${x}</span>`).join(""):"–"}</td><td>${t.sp}</td><td>${t.s}</td><td>${t.u}</td><td>${t.n}</td><td>${t.goals}</td><td>${t.diff>0?"+":""}${t.diff}</td><td class="points">${t.points}</td></tr>`).join("");
    document.querySelectorAll("[data-table-mode]").forEach(b=>b.classList.toggle("active",b.dataset.tableMode===mode));
    const heading=document.querySelector("#tableStage h2");if(heading)heading.textContent=({overall:"Aktuelle Tabelle",home:"Heimtabelle",away:"Auswärtstabelle",return:"Rückrundentabelle"})[mode]||"Aktuelle Tabelle";
  }
  function renderDashboard(){
    const live=matches().find(m=>["live","paused","halftime"].includes(m.status)),box=$("dashboardLiveMatch");if(!box)return;
    if(live){const s=parseScore(live);$("dashboardLiveTitle").textContent=`${live.home} vs ${live.away}`;$("dashboardLiveBadge").textContent=live.status==="live"?"LIVE":statusLabel(live.status);$("dashboardLiveBadge").classList.toggle("live-pulse",live.status==="live");box.classList.remove("skeleton-block");box.innerHTML=`<div class="dash-team">${teamLogo(live.home,true)}<strong>${esc(live.home)}</strong></div><div class="dash-score"><span>${s.h}</span><b>:</b><span>${s.a}</span><small>${esc(liveMinute(live))}</small></div><div class="dash-team">${teamLogo(live.away,true)}<strong>${esc(live.away)}</strong></div><button class="red-btn" data-match-open="${esc(live.id)}">Spiel öffnen</button>`}
    else{$("dashboardLiveTitle").textContent="Aktuell keine Live-Partie";$("dashboardLiveBadge").textContent="OFFLINE";$("dashboardLiveBadge").classList.remove("live-pulse");box.classList.remove("skeleton-block");box.innerHTML=`<div class="portal-empty">Sobald ein Spiel im Admin-Bereich auf <b>LIVE</b> steht, erscheint es hier prominent.</div>`}
    const settings=state.settings||{},lines=String(settings.nextStreams||"").split("\n").map(x=>x.trim()).filter(Boolean).map(line=>{const p=line.split("|").map(v=>v.trim());return {name:p[0]||"Stream",time:p[1]||"",url:p.slice(2).join("|")||""}});
    const sb=$("dashboardStreams");sb.classList.remove("skeleton-block");sb.innerHTML=lines.length?lines.map(x=>`<div class="stream-row"><div><strong>${esc(x.name)}</strong><span>${esc(x.time)}</span></div>${x.url?`<button class="ghost" data-stream-url="${esc(x.url)}">Stream</button>`:""}</div>`).join(""):`<div class="portal-empty">Noch keine Streams geplant. Im Admin-Bereich kannst du sie komplett pflegen.</div>`;
    const purl=String(settings.streamUrl||"").trim(),ptitle=String(settings.streamTitle||"VBL LIVE STREAM").trim();$("dashboardPlayer").innerHTML=purl?`<div class="stream-player-head"><strong>${esc(ptitle)}</strong><a href="${esc(purl)}" target="_blank" rel="noopener" class="ghost">Extern öffnen</a></div><div class="embed-shell"><iframe src="${esc(purl)}" title="${esc(ptitle)}" loading="lazy" allowfullscreen></iframe></div>`:"";
    const fav=favorites();$("favoriteArea").innerHTML=fav.length?fav.map(n=>{const t=getTeam(n);return `<button class="favorite-team-card"><span>${teamLogo(t||n,true)}</span><div><strong>${esc(n)}</strong><small>${Number(t?.points)||0} Punkte · ${Number(t?.sp)||0} Spiele</small></div><span class="star-big">★</span></button>`}).join(""):`<div class="portal-empty">Markiere ein Team mit ★, um es hier dauerhaft zu verfolgen.</div>`;
  }
  function renderLeagueVisuals(){
    const host=$("leagueVisuals");if(!host)return;
    const done=matches().filter(m=>m.status==="finished");
    const data=teams().map(t=>{const games=done.filter(m=>m.home===t.team||m.away===t.team);const gf=games.reduce((s,m)=>s+(m.home===t.team?parseScore(m).h:parseScore(m).a),0);const wins=games.filter(m=>{const s=parseScore(m);return m.home===t.team?s.h>s.a:s.a>s.h}).length;return {team:t.team,gf,wins,games}}).sort((a,b)=>b.gf-a.gf).slice(0,8);
    const max=Math.max(1,...data.map(x=>x.gf));
    host.innerHTML=data.length?data.map((x,i)=>`<div class="visual-row"><div class="visual-label"><span>${i+1}. ${esc(x.team)}</span><strong>${x.gf} Tore</strong></div><div class="progress-track"><span style="width:${Math.round(x.gf/max*100)}%"></span></div><small>${x.wins} Siege · ${x.games} Spiele</small></div>`).join(""):`<div class="portal-empty">Noch keine abgeschlossenen Spiele vorhanden.</div>`;
  }
  function renderRecordsEnhanced(){const finished=matches().filter(m=>m.status==="finished"),topGoals=finished.reduce((mx,m)=>Math.max(mx,parseScore(m).h+parseScore(m).a),0),biggest=finished.reduce((mx,m)=>{const s=parseScore(m);return(!mx||Math.abs(s.h-s.a)>Math.abs(parseScore(mx).h-parseScore(mx).a))?m:mx},null),streaks={};teams().forEach(t=>{const games=finished.filter(m=>m.home===t.team||m.away===t.team).sort((a,b)=>matchdayNumber(a)-matchdayNumber(b));let win=0,bestWin=0,un=0,bestUn=0;games.forEach(m=>{const s=parseScore(m),won=(m.home===t.team?s.h>s.a:s.a>s.h),draw=s.h===s.a;win=won?win+1:0;un=(won||draw)?un+1:0;bestWin=Math.max(bestWin,win);bestUn=Math.max(bestUn,un)});streaks[t.team]={bestWin,bestUn}});const maxWin=Math.max(0,...Object.values(streaks).map(x=>x.bestWin)),maxUn=Math.max(0,...Object.values(streaks).map(x=>x.bestUn));$("recordsGrid").innerHTML=[["HÖCHSTER SIEG",biggest?`${esc(biggest.home)} ${parseScore(biggest).h}:${parseScore(biggest).a} ${esc(biggest.away)}`:"–"],["MEISTE TORE IN EINEM SPIEL",topGoals?`${topGoals} Tore`:"–"],["LÄNGSTE SIEGESSERIE",maxWin?`${maxWin} Siege`:"–"],["LÄNGSTE UNGESCHLAGENE SERIE",maxUn?`${maxUn} Spiele`:"–"]].map(([a,b])=>`<div class="record-card record-animated"><span>${a}</span><strong>${b}</strong></div>`).join("")}
  function toast(msg){let host=$("toastHost");if(!host){host=document.createElement("div");host.id="toastHost";host.className="toast-host";document.body.appendChild(host)}const el=document.createElement("div");el.className="toast";el.textContent=msg;host.appendChild(el);setTimeout(()=>el.classList.add("show"),10);setTimeout(()=>{el.classList.remove("show");setTimeout(()=>el.remove(),250)},2600)}
  ...
  function renderTeams() {
    ...
    $("teamGrid").innerHTML = data.length ? data.map(t=>`<button class="team-pro-card" data-team-link="${esc(t.team)}"><div class="team-card-top">${teamLogo(t,true)}<span class="star-btn ${isFavorite(t.team)?"on":""}" data-fav-team="${esc(t.team)}">★</span></div><span>${esc(t.team)}</span><small>Trainer: ${esc(t.trainer||"–")}</small><strong>${Number(t.points)||0} P</strong></button>`).join("") : `<div class="portal-empty">Keine Teams gefunden.</div>`;
  }
  ...
  function renderAdminPlus() {
    ...
    if($("adminContact")){$("adminContact").value=state.settings.contact||"";$("adminDiscord").value=state.settings.discord||"";$("adminInstagram").value=state.settings.instagram||"";$("adminYoutube").value=state.settings.youtube||"";$("adminTiktok").value=state.settings.tiktok||"";$("adminStreamTitle").value=state.settings.streamTitle||"VBL LIVE STREAM";$("adminStreamUrl").value=state.settings.streamUrl||"";$("adminNextStreams").value=state.settings.nextStreams||"";}
  ...
  function bind() {
    ...
    document.querySelectorAll("[data-table-mode]").forEach(b=>b.onclick=()=>renderTableMode(b.dataset.tableMode));
    document.addEventListener("click",e=>{const b=e.target.closest("[data-share-team]");if(b)shareUrl(b.dataset.shareTeam);const fav=e.target.closest("[data-fav-team]");if(fav){e.preventDefault();e.stopPropagation();toggleFavorite(fav.dataset.favTeam)}const stream=e.target.closest("[data-stream-url]");if(stream)window.open(stream.dataset.streamUrl,"_blank","noopener");});
    ...
    setInterval(()=>{updateConnectivity();refreshLiveMinuteDom();renderDashboard();renderTableMode();renderLeagueVisuals();if(state.selectedSection==="players")renderPlayers();},1000);
  ...
  function setupFirebase() {
    ...
    listen('liga/settings','settings',()=>{renderInfo();renderDashboard();renderAdminPlus();});
  ...
  function start() {
    ...
    bind();
    setupFirebase();
    renderTeams();renderPlayers();renderStats();renderNews();renderArchive();renderInfo();renderAdminPlus();renderDashboard();renderTableMode();renderRecordsEnhanced();renderLeagueVisuals();updateConnectivity();
